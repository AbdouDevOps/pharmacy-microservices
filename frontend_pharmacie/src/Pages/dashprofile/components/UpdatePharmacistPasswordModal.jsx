...
const UpdatePharmacistPasswordModal = ({ open, onClose }) => {

  const [username, setUsername] = useState("");
  const [oldPassword, setOldPassword] = useState("");
  const [newPassword, setNewPassword] = useState("");

  const isDisabled = !username ||  !newPassword;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8081/api/pharmacists/admin/resetPassword",
        data,
        {
          headers: {
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            As an Admin Update a PharmacistPassword
            </Typography>
          </Box>
        ...
        >
          ...
          <FormControl>
            ...
            <Input
              ...
              placeholder="Enter Pharmacist new password"
              fullWidth
              sx={{
                borderRadius: 2,
                background: "rgba(255,255,255,0.9)",
...
export default UpdatePharmacistPasswordModal